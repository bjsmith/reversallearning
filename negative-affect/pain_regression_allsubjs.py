...
class RLPain:
    ...
    def process_detailed_regressors(self):
        for sid in range(1,500):
            for rid in [1,2]:
                nifti_file=self.fMRI_dir + '/sub'+str(sid) + 'ReversalLearningPunishrun'+str(rid)+'.nii.gz'
                if os.path.isfile(nifti_file):
                    print(self.fMRI_dir)
                    #got an nii.gz, check tosee if there's also a onset file for this.
                    onset_file=self.onset_dir + '/runfiledetail20170819T223713_s'+str(sid)+'_punishment_r'+str(rid)+'.txt'
                    if (os.path.isfile(onset_file)):
                        print ('we have a match!')

                        print(self.onset_file)
                        predicted_pain=self.get_trialtype_pain_regressors(nifti_file,onset_file)
