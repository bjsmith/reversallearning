...
do
	...
	#		/expdata/xfgavin/MSM/sub113/data
			...
			SUBJ_DIR="ReversalLearning_${m}_run${r}_pre.feat"
			SUBJECT_INPUT="/expdata/xfgavin/MSM/sub$s/analysis/${SUBJ_DIR}"
			if [ -e "$SUBJECT_INPUT" ];
			then
				...
				flirt -applyxfm -init ${SUBJECT_INPUT}/reg/standard2example_func.mat -in /expdata/bensmith/joint-modeling/code/wagertools/NPS_share/weights_NSF_grouppred_cvpcr.img -ref ${SUBJECT_INPUT}/filtered_func_data.nii.gz -out /expdata/bensmith/joint-modeling/code/wagertools/NPS_share/subject_space_masks/weights_NSF_grouppred_cvpcr_${SUBJ_DIR}
			fi
		done
	done
done
