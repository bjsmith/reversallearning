...
models_to_run<-c("double_update_notrialpost",#"double_update_nov_rev2-d",#"double_update_nov_rev2-c",
                 "double_update_nov_rev2-a","double_update_nov_rev2")
...
times_to_run<-4
#run.
summaryfilepath<-paste0(localsettings$data.dir,"du_model_revised_4runs_20171206.RData")

models.with.4.separate.runs.count<-1
models.with.runs.considered.together.count<-2
total.models.count<-length(subject_groups)*times_to_run*(models.with.4.separate.runs.count*4+models.with.runs.considered.together.count)
          ...
          for (rp in rp_list){
            for(runs in runlist){
              ...
              fitpackage<-lookupOrRunFit(
                ...
                fastDebug=TRUE,
                fileSuffix=paste0("rev2_20171206",as.character(t)),
                estimation_method = em,
                bseed=t+1799147219,#set.seed(as.numeric(Sys.time())); sample.int(.Machine$integer.max, 1)
              ...
              # first_empty_list_pos<-min(which(sapply(model.summaries,is.null)))
              # print(first_empty_list_pos)
              # cat("\n")
              # model.stanfits[[first_empty_list_pos]]<-"fitpackage$fit"
              # model.summaries[[first_empty_list_pos]]<-"list(\"summaryObj\"=data_summarize_double_update_rev2_repeated_runs(..."
            }
          }
        }
      }
