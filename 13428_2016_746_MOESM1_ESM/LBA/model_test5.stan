//v4: can the choleksy prior be less than zero?
data{
  int covar_size;
  int N;
...
model{
  ...
  L_Omega ~ lkj_corr_cholesky(0); //parameter was 4 in manual but using 1 because our variables are individually normally distributed
  L_sigma ~ cauchy(0,sigma_diag_gamma); //gamma was 2 in manual but using 1 because our variables are normally distributed
  modeled_data ~ multi_normal_cholesky(zeros,L_Sigma);
