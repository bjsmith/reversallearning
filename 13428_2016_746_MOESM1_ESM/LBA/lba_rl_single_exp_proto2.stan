...
transformed data{
  ...
  for (i in 1:LENGTH){
    ...
    if(response[i]==required_choice[i]){
      for (j in 1:NUM_CHOICES){
        if(j==response[i]){
          choice_outcomes[i,j]=1;
        }else{
          choice_outcomes[i,j]=-1.0/(NUM_CHOICES-1);
        }
      }
    }else{//response was NOT the required choice
      for (j in 1:NUM_CHOICES){
        if(j==response[i]){
          choice_outcomes[i,j]=-1;
        }else{
...
transformed parameters {
  ...
  real testBoolVarPos1 = ((2==2)-0.5)*2;
  real testBoolVarNeg1 = ((1==2)-0.5)*2;
  s = 1;
...
model {
  ...
  alpha_pr ~ normal(0,1);//normal(-3,3);//weak prior for no learning.  
  k_pr ~ normal(log(.5),1);
  //A ~ normal(.5,1)T[0,];
