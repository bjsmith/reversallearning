...
if(plot.phi==TRUE){
  ...
  if(exists(l2.groups.list)){
    phi_names<-paste0(rep(hpar.names,times=3,each=1)," ",rep(l2.groups.list,times=1,each=6))
  }
  for(k in 1:length(phi_names)){
    ...
    matplot(t(phi[,k,start:tnmc]),type="l",lty=1,main="",ylab=phi_names[k])
    hist(phi[,k,start:tnmc],prob=T,breaks=breaks,main="",xlab=phi_names[k])
    if(plot.priors==TRUE){
      lines(xs,ys,lty=2)
      count=count+1
    }}}
