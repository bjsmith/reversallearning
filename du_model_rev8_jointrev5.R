...
#set settings.
models_to_run<-c("double_update_jointrev5","double_update_rev8")
...
times_to_run<-1
#run.
summaryfilepath<-paste0(localsettings$data.dir,"du_model_rev8_v_jointrev1_both.RData")
      ...
      for (m in models_to_run){#m<-models_to_run[1]
        for (g in subject_groups){g<-1
          ...
          if(m %in% c("double_update_rpo_repeated_runs_notrialpost", "double_update_rpo_repeated_runs_ntp_otmod",
                      ...
                      "double_update_jointrev1","double_update_jointrev2","double_update_jointrev5")){
            runlist<-list(c(1,2))
            rp_list<-list(c(1,2))
                ...
                model.summaries[[first_empty_list_pos]]<-
                  list("summaryObj"=data_summarize_double_update_rev4(extractedfit,comprehensive=FALSE,
                       ...
                       "g"=g,"m"=m,"t"=t,"EstimationMethod"=em,"elapsedTime"=fitpackage$general_info$estimation_duration)
                                ...
                                "double_update_jointrev1","double_update_jointrev2",
