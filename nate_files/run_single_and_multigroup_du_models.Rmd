...
```{r double_update_reward_and_punishment_multiple_speed_improvement, echo=TRUE}

fit.RiskyMeth<-lookupOrRunFit(run=c(1,2),groups_to_fit=3, 
                              model_to_use="double_update_rpo_repeated_runs",includeSubjGroup = FALSE,
                              rp=c(REVERSAL_LEARNING_REWARD,REVERSAL_LEARNING_PUNISHMENT),
                              model_rp_separately=TRUE,model_runs_separately = TRUE, include_pain=FALSE)
```
...
```
These don't seem particularly informative and I suspect there is an error with the way they ahve been calculated. In particular, ratios for "alpha" (learning rate) and "beta" (inverse temperature) seem identical. There's got ot be something wrong here.
