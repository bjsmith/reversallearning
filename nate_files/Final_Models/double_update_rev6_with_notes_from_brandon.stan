    ...
    for (t in 1:(Tsubj[s])) {
      ...
      if (choice[s,t]!=0) {#non-response trial.
        if(sample_from_prior!=1){
          ...
          //cue array stores the image ID for each subject for each trial
          //then we use that ID to look up the expected value for that imagein the ev array
          //EV array is a nx2 matrix where n is the number of stimuli in the task.
