...
model {
  ...
  for (ot in OT_MIN:OT_MAX){//cycle through outcome types
  alpha_pr[,ot]  ~ normal(0,1);
  beta_pr[,ot]   ~ normal(0,1);
  }
  //beta_pun_pr   ~ normal(0,1);

  //an array of run modifiers. The first element has modifiers for run2;  
  //the second for run 3, and so on; all relative to run 1
  //of course in this dataset we only have two runs, 
  //but the design here will be extensible for more than two runs :-)
  for (r in 1:(R-1)){
  	for (ot in OT_MIN:OT_MAX){
  	    alpha_pr_rm[r,,ot]  ~ normal(0,1);
    	beta_pr_rm[r,,ot]   ~ normal(0,1);
  	}
  }
