//jointrev4: copied from jointrev1, uses simple correlation rather than covariance.
...
// Declare all parameters as vectors for vectorizing
  ...
  cholesky_factor_corr[TD_N] Sigma;
...
model {
  ...
  for (r in 1:NUM_RUNS){
    ...
    theta_delta[:,THETA_rpe]=trial_PE;
    theta_delta[:,THETA_ev]=trial_ev;
    ...
    // print(zeros);
    // print(L_Sigma);
    td_var ~ multi_normal_cholesky(zeros,Sigma);//sample the ThetaDelta matrix from a cholesky matrix
    ////////////\end{joint model machinery}
  }
