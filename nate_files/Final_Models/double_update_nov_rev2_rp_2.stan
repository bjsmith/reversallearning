...
//need to adapt this somehow to allow for 'shrinkage' of the reward and punishment values across SUBJECTS.
data {
  int<lower=1> N;
  int<lower=1> T;
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N,OT_MAX-OT_MIN+1] alpha_ot_pr;   // learning rate
  vector[N,OT_MAX-OT_MIN+1] beta_ot_pr;  // inverse temperature
}
...
transformed parameters {
  ...
  vector<lower=0,upper=1>[N,OT_MAX-OT_MIN+1] alpha_ot;
  vector<lower=0,upper=5>[N,OT_MAX-OT_MIN+1] beta_ot;
  ...
  for (i in 1:N) {//iterate through subjects AND runs
    for (r in 1:R){
      ...
      alpha_r[i,r]  = Phi_approx( 
        ...
        alpha_pr_sigma_ot[i] * alpha_ot_pr[i,r]); #not sure whether to use mu_p_r or alpha_r values here.
        
      beta_r[i,r]   = Phi_approx( 
        mu_p[2] + 
        sigma[2] * beta_pr[i] + 
        beta_pr_sigma[i] * beta_r_pr[i,r]+
        beta_pr_sigma_ot[i] * beta_ot_pr[i,r]) * 5; #not sure whether to use mu_p_r or alpha_r values here.
    }
...
model {
  ...
  alpha_pr_sigma_ot ~ cauchy(0, 5);
  beta_pr_sigma_ot ~ cauchy(0, 5);
  ...
  //outcome type (reward/punishment) parameters
  alpha_pr_ot  ~ normal(0,1);
  beta_pr_ot   ~ normal(0,1);
