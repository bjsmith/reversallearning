#second attempt at a vectorizing reward/punishment differences
#this time, will just try to gently increment it.
#on hold until we sort out more basic problems!
data {
...
transformed parameters {
  ...
  for (s in 1:N) {
    alpha[s,OT_REW]  = Phi_approx( mu_p[OT_REW, 1] + sigma[OT_REW, 1] * alpha_pr[s,OT_REW] );
    beta[s,OT_REW]   = Phi_approx( mu_p[OT_REW, 2] + sigma[OT_REW, 2] * beta_pr[s,OT_REW] ) * 5;
    
    alpha[s,OT_PUN]  = Phi_approx( mu_p[OT_PUN, 1] + sigma[OT_PUN, 1] * alpha_pr[s,OT_PUN] );
    beta[s,OT_PUN]   = Phi_approx( mu_p[OT_PUN, 2] + sigma[OT_PUN, 2] * beta_pr[s,OT_PUN] ) * 5;
    #I am not sure we need to transform the multipliers? They stand on their own, perhaps...
    for (r in 1:(R-1)){
      alpha_run_multiplier[r,s,OT_REW]  = Phi_approx( mu_p_rm[OT_REW, r, 1] + sigma_rm[OT_REW, r, 1] * alpha_pr_run_multiplier[r,s,OT_REW] );
      beta_run_multiplier[r,s,OT_REW]   = Phi_approx( mu_p_rm[OT_REW, r, 2] + sigma_rm[OT_REW, r, 2] * beta_pr_run_multiplier[r,s,OT_REW] ) * 5;
      
      alpha_run_multiplier[r,s,OT_PUN]  = Phi_approx( mu_p_rm[OT_PUN, r, 1] + sigma_rm[OT_PUN, r, 1] * alpha_pr_run_multiplier[r,s,OT_PUN] );
      beta_run_multiplier[r,s,OT_PUN]   = Phi_approx( mu_p_rm[OT_PUN, r, 2] + sigma_rm[OT_PUN, r, 2] * beta_pr_run_multiplier[r,s,OT_PUN] ) * 5;    
    }
  }
}
...
model {
  ...
  alpha_pr[,OT_REW]  ~ normal(0,1);
  alpha_pr[,OT_PUN]  ~ normal(0,1);
  beta_pr[,OT_REW]   ~ normal(0,1);
  beta_pr[,OT_PUN]   ~ normal(0,1);
  ...
  for (r in 1:(R-1)){
    alpha_pr_run_multiplier[r,,OT_REW]  ~ normal(0,1);
    alpha_pr_run_multiplier[r,,OT_PUN]  ~ normal(0,1);
    beta_pr_run_multiplier[r,,OT_REW]   ~ normal(0,1);
    beta_pr_run_multiplier[r,,OT_PUN]   ~ normal(0,1);
  }
  ...
  for (s in 1:N) {
    ...
    for (t in 1:(Tsubj[s])) {
      ...
      if (choice[s,t]!=0) {
        ...
        alpha_s=alpha[s,outcome_type[s,t]];
        beta_s=beta[s,outcome_type[s,t]];
        if(run_id[s,t]>1){#only run if run_id is greater than 1; otherwise we fix the multiplier to 1, by default.
          alpha_pr_run_multiplier_s=alpha_pr_run_multiplier[run_id[s,t]-1,s,outcome_type[s,t]];
          beta_pr_run_multiplier_s=beta_pr_run_multiplier[run_id[s,t]-1,s],outcome_type[s,t];
        }
