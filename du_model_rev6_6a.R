#this partial version will only load models that have already been run.
#Purposes:
    ...
    for (t in 1:times_to_run){
      for (m in models_to_run){
        for (g in subject_groups){
          print (paste0("em:",em, "; t:",t,"; m:",m,"; g:",g,collapse=", "))
          ...
          for (rp in rp_list){
            for(runs in runlist){
              current_list_pos<-current_list_pos+1 #this counter must appear within the innermost loop and must be run each time.
              print(paste("current_list_pos is", as.character(current_list_pos)))
              ...
              if (is.null(model.summaries[[current_list_pos]])){
                ...
                if(!is.na(fitpackage)){
                  ...
                  first_empty_list_pos<-current_list_pos
                  ...
                  print ("Model has not been estimated. Because this is set to only lookup existing runs, it hasn't been estimated.")
                }
              }else{
                print("model already in summary list.")
              }
            }
          }
        }
