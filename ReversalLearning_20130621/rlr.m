% function ReversalLearning_reward(subjectID,run)
...
eval(sprintf('RL(:,Mtrial)=run%d(:,1);',run));
...
[w, rect]=Screen('OpenWindow', s, bkGround,[0 0 649 480], 32);
HideCursor;
...
inst={{['RLR Task--' 'Run(' num2str(run) ')']}...
      ...
      {''}...
      {''} ...
      {'Your goal is to EARN MORE POINTS.'}...
      {''} ...
      {'Please make your response FAST'}...
      {''} ...
      {'Please keep your head very STILL'};
};

for x=1:size(inst,2),
    Screen('DrawText',w,inst{x}{:},100,100+x*28,white);
end;
