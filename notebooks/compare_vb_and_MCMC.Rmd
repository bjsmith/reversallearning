---
title: "testing out stan MCMC vs. variational bayes"
author: "Ben Smith"
date: "9/27/2017"
...
for(ms.i in 1:length(model.summaries)){
  ...
  if(mod(length(ms$summaryObj$iter),3992)==0){
    ms.summaryObj$EstimationMethod=as.character(ESTIMATION_METHOD.VariationalBayes)
    
  }else if (mod(length(ms$summaryObj$iter),24000)==0){
...
```{r VBReliability, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
```{r MCMCReliability, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
```{r StanTracePlot, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
#
```
Once again, these all look very stable.

```{r StanPlotLogResults, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
#class(model.stanfits[[5]])
...
```{r StanPlotDensity, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
#class(model.stanfits[[5]])
...
```{r StanGeneralStats, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
#class(model.stanfits[[5]])
...
```{r StanAutocorrelation, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
```{r StanMCSE, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
for (i in 1:length(model.summaries)){
  if(model.summaries[[i]]$EstimationMethod=="MCMC" & model.summaries[[i]]$t==1){
...
```{r VBTraceplot, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
for (i in 1:length(model.summaries)){
...
```{r VBStanGeneralStats, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
#class(model.stanfits[[5]])
...
```{r MCMCresultsGroupComparison, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
```{r MCMCresultsRunComparison, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
...
```{r MCMCresultsMotivationComparison, echo=TRUE, message=FALSE, warning=FALSE,cache=TRUE, fig.cap=TRUE}
