...
#3) Implemented my own innovation using empirical priors and empirical initial values.
...
#set settings.
models_to_run<-c("double_update_rev8","double_update_rev5a")
          ...
          for (rp in rp_list){
            for(runs in runlist){
              ...
              fitpackage<-lookupOrRunFit(
                ...
                fastDebug=FALSE,
                fileSuffix=paste0("rev_20180618",as.character(t)),
                estimation_method = em,
                bseed=t+1652858056,#set.seed(as.numeric(Sys.time())); sample.int(.Machine$integer.max-1000, 1)
