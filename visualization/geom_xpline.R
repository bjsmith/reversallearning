...
#'
#' \if{html}{
#' A sample of the output from \code{geom_splurgle()}:
#'
#' \figure{geomsplurgle01.png}{options: width="100\%" alt="Figure: geomsplurgle01.png"}
#' }
#'
#' \if{latex}{
#' A sample of the output from \code{geom_splurgle()}:
#'
#' \figure{geomsplurgle01.png}{options: width=10cm}
#' }
#'
#' @section Aesthetics:
#' \code{geom_splurgle} understands the following aesthetics (required aesthetics
#' are in bold):
#' \itemize{
#'   \item \strong{\code{x}}
#'   \item \strong{\code{y}}
...
#'  \code{\link[graphics]{splurgle}};
#'  \code{\link[grid]{grid.splurgle}}
#'
#' @details
...
#'   \code{geom_splurgle} and \code{stat_splurgle}.
...
#' @export
#' @family splurgle implementations
#' @examples
#' set.seed(1492)
...
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_splurgle(size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_splurgle(spline_shape=-0.4, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
...
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_splurgle(spline_shape=1, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_splurgle(spline_shape=0, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_splurgle(spline_shape=-1, size=0.5)
geom_splurgle <- function(mapping = NULL, data = NULL, stat = "splurgle",
...
#' @keywords internal
#' @export
GeomSplurgle <- ggproto("GeomSplurgle", GeomLine,
                       required_aes = c("x", "y"),
                       default_aes = aes(colour = "black", size = 0.5, linetype = 1, alpha = NA)
)

#' @export
#' @rdname geom_splurgle
#' @section Computed variables:
#' \itemize{
#'   \item{x}
#'   \item{y}
#' }
stat_splurgle <- function(mapping = NULL, data = NULL, geom = "line",
...
#' @usage NULL
#' @export
StatSplurgle <- ggproto("StatSplurgle", Stat,
