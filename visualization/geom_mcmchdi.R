...
#'
#' \if{html}{
#' A sample of the output from \code{geom_mcmchdi()}:
#'
#' \figure{geommcmchdi01.png}{options: width="100\%" alt="Figure: geommcmchdi01.png"}
#' }
#'
#' \if{latex}{
#' A sample of the output from \code{geom_mcmchdi()}:
#'
#' \figure{geommcmchdi01.png}{options: width=10cm}
#' }
#'
#' @section Aesthetics:
#' \code{geom_mcmchdi} understands the following aesthetics (required aesthetics
#' are in bold):
#' \itemize{
#'   \item \strong{\code{x}}
#'   \item \strong{\code{y}}
...
#'  \code{\link[graphics]{mcmchdi}};
#'  \code{\link[grid]{grid.mcmchdi}}
#'
#' @details
...
#'   \code{geom_mcmchdi} and \code{stat_mcmchdi}.
...
#' @export
#' @family mcmchdi implementations
#' @examples
#' set.seed(1492)
...
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_mcmchdi(size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_mcmchdi(spline_shape=-0.4, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
...
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_mcmchdi(spline_shape=1, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_mcmchdi(spline_shape=0, size=0.5)
#'
#' ggplot(dat, aes(x, y, group=group, color=factor(group))) +
#'   geom_point(color="black") +
#'   geom_smooth(se=FALSE, linetype="dashed", size=0.5) +
#'   geom_mcmchdi(spline_shape=-1, size=0.5)
geom_mcmchdi <- function(mapping = NULL, data = NULL, stat = "mcmchdi",
...
#' @keywords internal
#' @export
GeomMCMCHDI <- ggproto("GeomMCMCHDI", GeomSegment,
                       required_aes = c("x", "y"),
...
#'   \item{y}
#' }
stat_mcmchdi <- function(mapping = NULL, data = NULL, geom = "Segment",
...
#' @usage NULL
#' @export
StatMCMCHDI <- ggproto("StatMCMCHDI", Stat,
                       ...
                       compute_group = function(self, data, scales, params,
                         ...
                         # 
                         require(hBayesDM)
                         mcmchdivals<-HDIofMCMC(data$x)
                         data.frame(x=mcmchdivals[1],xend=mcmchdivals[2],y=0,yend=0)
                       }
)