...
#set settings.
models_to_run<-c("double_update_nov_rev2")
    ...
    for (t in 1:times_to_run){
      for (m in models_to_run){
        for (g in subject_groups){
          ...
          if(m %in% c("double_update_rpo_repeated_runs_notrialpost", "double_update_rpo_repeated_runs_ntp_otmod","double_update_nov_rev2")){
            runlist<-list(c(1,2))
            rp_list<-list(c(1,2))
          ...
          for (rp in rp_list){
            for(runs in runlist){
              ...
              fitpackage<-lookupOrRunFit(
                ...
                fastDebug=FALSE,
                fileSuffix=paste0("rev2_20171016",as.character(t)),
