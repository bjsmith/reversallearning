model.name<-"rl_discrim_single_exp_joint_v10_testing"
library(rstan)
...
lba_rl_version<-"rl_joint_discrim_20181015_1"
...
Rhat_corevals_limit=1.05 #
Rhat_corevals_count<-2
...
model.vector<-list()
#rl_single_exp_joint_v10: equivalent, but with no discrimination model.
#rl_discrim_single_exp_joint_v5: v4, but with a critical bug fixed.
...
#rl_discrim_single_exp_joint_v9: v5, but applies a separate learning rate to the discrimination and value learning
#rl_discrim_single_exp_joint_v10: v5, uses a full mixture model.
failure_count<-0
n_chains=6
          ...
          if(!is.null(srm.fit)){
            ...
            test_rhat_vals<-function(rhat_vector){
              ...
              if (all(rhat_vector[!is.nan(rhat_vector)]<=Rhat_general_limit)){
                if(all(rhat_vector[1:Rhat_corevals_count]<=Rhat_corevals_limit)){
                  return(TRUE)
                }
              }
              return(FALSE)
            ...
            if(test_rhat_vals(summary(srm.fit)$summary[,"Rhat"])){
              ...
              iterations=iterations+warmup*0.4
              warmup=warmup+warmup*0.4
