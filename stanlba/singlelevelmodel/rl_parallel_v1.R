...
lba_rl_version<-"rl_joint_20180628_1"
...
model.name<-"rl_single_exp_parallel"
cat("Compiling model...")
            ...
            expr = {
              withTimeout({
                sampling(lba_rl_single_joint, 
                         data = create_standatalist_parallelmodel(srm.data = srm.data),
                         warmup = warmup, 
                         iter = iterations,
                         init=get_starting_values(n_chains,theta_count=4),
                         chains = n_chains,
