...
lba_rl_version<-"joint_20180628_1"
...
results_summary_list_filepath<-paste0(output_dir,"run_package_summary_joint_v2_",lba_rl_version,".RData")
grand_posterior_estimate_filepath<-paste0(output_dir,"posteriors_joint_v2_",lba_rl_version,".RData")
...
model_name<-"lba_rl_single_exp_joint_v2"
...
for (sid in unique(rawdata$subid)){#sid<-106
  for (r in unique(rawdata[subid==sid,runid])){#r<-1
    for(m in unique(rawdata[subid==sid & runid==r,Motivation])){#m<-"reward"
      ...
      package_filepath<-paste0(output_dir,"run_package_",sid,"_",r,"_",m,"_",model_name,"_joint_v1.RData")
