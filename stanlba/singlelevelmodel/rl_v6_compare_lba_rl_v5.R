...
lba_rl_version<-"rl_joint_20180701_1"
...
results.list<-list()
#model.name.vector<-c("rl_single_exp_joint_v6","rl_single_exp_parallel_v6","lba_rl_single_exp_joint_v5","lba_rl_single_exp_parallel_v5")
  ...
  for (r in unique(rawdata[subid==sid,runid])){#r<-1
    ...
    for(m in motivations){#m<-"punishment"
      ...
      if(length(grep("joint",model.name))==1){
        ...
        print("couldn't determine the model category")
      }
