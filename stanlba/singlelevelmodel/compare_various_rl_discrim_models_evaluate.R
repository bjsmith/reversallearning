#rl_discrim_single_exp_joint_v6: v5, but with an EV update weighted by the familiarity score, still updating on the actual item.
#rl_discrim_single_exp_joint_v7: v5, but with multiplicative rather than additive familiarity score.
#rl_discrim_single_exp_joint_v8: v5, but with a simple weighting so that the familiarity score maxes out at 1 (i.e., 12 exposures to every item)
#rl_discrim_single_exp_joint_v9: v5, full working mixture model???
#rl_discrim_single_exp_joint_v10: v5, mixture model for estimation (though not for joint modeling)
...
lba_rl_version<-"rl_joint_discrim_20180914_1"
...
model_rl_discrim_v5<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version=lba_rl_version,
  model_filename="rl_discrim_single_exp_joint_v5",
  model_subversion="")

model_rl_discrim_v6<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version=lba_rl_version,
  model_filename="rl_discrim_single_exp_joint_v6",
  model_subversion="")

model_rl_discrim_v7<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version=lba_rl_version,
  model_filename="rl_discrim_single_exp_joint_v7",
  model_subversion="")

model_rl_discrim_v8<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version=lba_rl_version,
  model_filename="rl_discrim_single_exp_joint_v8",
  model_subversion="")

model_rl_discrim_v9<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version=lba_rl_version,
  model_filename="rl_discrim_single_exp_joint_v9",
  model_subversion="")
...
model_rl_discrim_v10<-single_level_model_summarize_fast(
  single_run_dir=paste0(localsettings$data.dir,"lba_rl"),
  model_version='rl_joint_discrim_20181015_1',
  model_filename="rl_discrim_single_exp_joint_v10",
  model_subversion="a")

models <- list("control_v10"=model_rl_control_v10,
               "v5"=model_rl_discrim_v5,
#             "v6"=model_rl_discrim_v6,
             "v7"=model_rl_discrim_v7,
             "v8"=model_rl_discrim_v8,
             "v9"=model_rl_discrim_v9,
              "v10"=model_rl_discrim_v10
              )
...
for (model_name in names(models)){#model_name<-"v9"
  ...
  params_to_check<-c("alpha_pr","beta_pr","alpha_pr_discrim", "alpha","beta","alpha_discrim")
...
#so nothing particularly distinguishes any of the improvements v7 through v9, and in fact, it's a little worrying how little difference 
#it made to, say, change the scale of the estimated values.
#however v10 looks much better! Let's look into this more.
...
# 
# 
# load("/expdata/bensmith/joint-modeling/data/msm/reversallearning/lba_rl/rl_joint_discrim_20180914_1/run_package_107_1_reward_rl_discrim_single_exp_joint_v10.RData")
# extdata<-rstan::extract(run_package$fit)
# delta_names<-c("Accum1","Accum2","OFC1","OFC2")
# heatmap(get_Sigma_m_n(model_rl_control_v10$results_summary_dt,theta_names,delta_names))

#so three problems here:
# (1) why is the log_lik score so high for the mixture model? Is it coded the same way as the others?
# (2) the control might not be a fair comparison because it treats calculation of the mean differently.
# (3) what I can tell with the estimates from these is that the individual-trial-level estimates might be *really really* poor.
