...
lba_rl_version<-"rl_opal_joint_20180709_1"
...
results.list<-list()
model.name<-"rl_opal_single_exp_joint_v8b"
model.subversion<-"b"
#model.name.vector<-c("rl_single_exp_joint_v6","rl_single_exp_parallel_v6","lba_rl_single_exp_joint_v6","lba_rl_single_exp_parallel_v5")
#for (model.name in model.name.vector){#model.name<-"rl_single_exp_joint_v6"
cat("Compiling model...")
compiled_model<-stan_model(paste0('stanlba/stanfiles/incremental/',model.name,'.stan'))
cat("compiled.\n")
failure_count<-0
  ...
  for (r in unique(rawdata[subid==sid,runid])){#r<-1
    ...
    for(m in motivations){#m<-"punishment"
      ...
      if(length(grep("joint",model.name))==1){
        model.data<-create_standatalist(srm.data = srm.data,theta_count = 1)
        thetaDelta_count=5
      ...
      if(length(grep("lba_rl",model.name))==1 | length(grep("rl_opal",model.name))==1){
        model.init<-get_starting_values(n_chains,thetaDelta_count=thetaDelta_count)
      }else{
        model.init<-get_starting_values_alphabeta(n_chains,thetaDelta_count=thetaDelta_count)
      }
