...
Rhat_limit=10e8 # I don't care about this at the moment. I just want to take a look at what we're getting!
results.list<-list()
#model.name<-"lba_rl_single_exp_joint_v1"
#model.name<-"lba_rl_single_exp_joint_v1_trimmed_4"
model.name<-"lba_rl_single_exp_joint_v2"
cat("Compiling model...")
...
for (sid in unique(rawdata$subid)){#[unique(rawdata$subid)>ll & unique(rawdata$subid)<=ul]){#c(105,106,107)){#unique(rawdata$subid)[1:3]){#sid<-106
  for (r in unique(rawdata[subid==sid,runid])){#r<-1
    for(m in unique(rawdata[subid==sid & runid==r,Motivation])){#m<-"punishment"
      package_filepath<-paste0(output_dir,"run_package_",sid,"_",r,"_",m,"_",model.name,"_joint_v1.RData")
      srm.data<-select_rawdata_cols_for_run(rawdata,sid,r,m)
      if(!file.exists(package_filepath)){
        model_attempts=0
        #warmup=200
        #iterations=300
        warmup=160;iterations=200
        try_this_model=TRUE
        while(try_this_model==TRUE){
          ...
          srm.fit <- tryCatch(
            expr = {
              ...
              },timeout = 60*60*24,onTimeout = "error")
            }, 
            TimeoutException = function(ex){cat(paste0("could not run calculation for sid",sid," rid", r, " m", m, " within 24 hours. skipping!"))
            cat(ex)
            cat("\n")
            return(NULL)
            }
          ...
          if(!is.null(srm.fit)){
            ...
            test_rhat_vals<-function(rhat_vector){
              ...
              if (all(rhat_vector[!is.nan(rhat_vector)]<=Rhat_limit))return(TRUE)
              return(FALSE)
            }
