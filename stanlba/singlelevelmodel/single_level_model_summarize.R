...
single_level_model_summarize <- function(
  ...
  for (sid in unique(rawdata$subid)){#sid<-106
    for (r in unique(rawdata[subid==sid,runid])){#r<-1
      for(m in unique(rawdata[subid==sid & runid==r,Motivation])){#m<-"reward"
        ...
        if(list_counter%%20==0 & length(posterior_estimate_list)>0){
          
          #once eveery 20 runs...
          if(is.null(grand_posterior_estimate))grand_posterior_estimate<-rbindlist(posterior_estimate_list,fill=TRUE)#do.call(rbind,posterior_estimate_list)
          else grand_posterior_estimate<-rbindlist(list(grand_posterior_estimate,rbindlist(posterior_estimate_list,fill=TRUE)),fill=TRUE)
  ...
  #...and once at the end.
  if(is.null(grand_posterior_estimate))grand_posterior_estimate<-do.call(rbind,posterior_estimate_list,args = )
  else grand_posterior_estimate<-rbind(grand_posterior_estimate,do.call(rbind,posterior_estimate_list))
  save(grand_posterior_estimate,file=grand_posterior_estimate_filepath)
