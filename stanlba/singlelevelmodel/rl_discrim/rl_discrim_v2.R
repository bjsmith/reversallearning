...
source("stanlba/singlelevelmodel/rl_discrim/rl_discrim_joint_v1_functions.R")

# round 101; cue:63; cue EV:[0,0]
# distractor cue set:
#   [60,64,62]
# distractor cue EVs
# [[0.67519,-0.67519],[-0.291678,0.291678],[-0.290349,0.290349]]
# pr(recognition):0.623527; exp_val_g_pr_recognition:[0.011691,-0.011691]
#distractor_evs<-t(matrix(c(0.67519,-0.67519,-0.291678,0.291678,-0.290349,0.290349),ncol=3))
#distractor_sum<-t(matrix(rep(1,3),nrow=3)) %*% distractor_evs
#distractor_sum==rowSums(distractor_evs)
#pr_recognition=0.623527
#pr_recognition*c(0,0)+(1-pr_recognition)/3*distractor_sum


# round 132; cue:65; cue EV:[0.644621,-0.644621]
# distractor cue set:
#   [63,65,66]
# distractor cue EVs
# [[-0.352606,0.352606],[0.644621,-0.644621],[-0.644621,0.644621]]
# pr(recognition):0.623527; exp_val_g_pr_recognition:[0.35769,-0.35769]

#add rankings to the raw data so that we know the relative timings of the events
...
# View(rawdata[subid %in% 106:109,.(subid,runid,Motivation,presentation_n_over_segments, onset_time_actual,presentation_n_over_run,cue)])
...
lba_rl_version<-"rl_discrim_joint_20180702_2"
...
model.name<-"rl_discrim_single_exp_joint_v2"
cat("Compiling model...")
lba_rl_single_joint<-stan_model(paste0('stanlba/stanfiles/incremental/',model.name,'.stan'))
cat("compiled.\n")
failure_count<-0

#View(rawdata[subid %in% 106:109,.(subid,runid,Motivation,presentation_n_over_segments, onset_time_actual,presentation_n_over_run)])
            ...
            expr = {
              withTimeout({
                sampling(lba_rl_single_joint, 
                         ...
                         warmup = warmup, 
                         iter = iterations,
                         init=get_starting_values_alphabeta(n_chains,theta_count=5),
                         chains = n_chains,
