...
lba_rl_version<-"rl_opal_joint_20180708_1"
...
results.list<-list()
model.name<-"rl_opal_single_exp_joint_v8.stan"
...
#for (model.name in model.name.vector){#model.name<-"rl_single_exp_joint_v6"
  ...
  if(length(grep("lba_rl",model.name))==1){
    model.init<-get_starting_values(n_chains,theta_count=theta_count)
  }else{
    model.init<-get_starting_values_alphabeta(n_chains,theta_count=theta_count)
  }
