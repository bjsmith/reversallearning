...
multisubj_multirun_moresubs<-rawdata[subid %in% c(105:120) #& Motivation=="reward" 
                                     & reaction_time>0,
...
save(fit_normalsds,fit_widevariablecauchysfit_base,file=paste0(dd, "Fits/hierarchical_stanforum_suggestion_results.RData"))

#based on intial transition estimates, going from 2s*2r/s=4r to 15s*4r/s=60r has increased the "1000 transitions" estimate from 11 s to 284 s. For the final model we'd be hoping to increase iterations by a factor of 20 and subjects by a factor of 10, so our estimate should be that the final model will take 200x as long, 
#i.e., if we aim for a 168 hour limit, this practice should take no longer than 1.7 hours.
