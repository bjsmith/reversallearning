//v5: fixed what I believe was a critical bug that messed up calculation of expected value in the discrimination
//
//rl_single_exp_joint
...
transformed data{
  ...
  int n_back=3;
  int max_cue_presentations=max(cue_presentation_n);
...
model {
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    if(trial_n_back>0){
      ...
      match_score[1:trial_n_back]=inv_logit((cue_match[1:trial_n_back]*2-1).*recent_cue_discriminability[1:trial_n_back]/max_cue_presentations*alpha);
