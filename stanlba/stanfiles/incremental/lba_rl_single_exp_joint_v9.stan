...
model {
  ...
  // //I've got no interest in actually calculating means here, so I think that's a better approach.
  for (tdi in 1:TD_N){
...
//   vector[NUM_CHOICES] v;
//   
//   matrix[TD_N,TD_N] td_corr;
//   matrix[TD_N,TD_N] inv_mat_var;
//   
//   
//   ///////MODEL REPEAT
//   ///THE code below repeats the model above.
...
//     for(j in 1:NUM_CHOICES){
//       v[j]=logit(exp_val[cue[i],j]/4+0.75);
//       run_pred_err_c2[i]=choice_outcomes[i,j]-exp_val[cue[i],j]; 
//       exp_val[cue[i],j] = exp_val[cue[i],j] + alpha*run_pred_err_c2[i];
//     }
//     //we don't do this in the generated quantities.
//     //response_time[i] ~ lba(response[i],k,A,v,s,tau);
//   }
//   
//   //////////////CORRELATION MATRIX
