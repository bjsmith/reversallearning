//v7: v5, but with multiplicative rather than additive familiarity score.
//rl_single_exp_joint
...
model {
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    if(trial_n_back==0){
      ...
      recent_cue_familiarity[1] = cue_presentation_n[i-2];
    ...
    if(trial_n_back>0){
      recent_cue_discriminability[1:trial_n_back] = 
        ...
        rep_vector(cue_presentation_n[i],trial_n_back);
      //here, discriminability is the product of the familiarity score of the cue and the trial
      //other versions use the sum, not the product. This version also avoids decrementing by 1, to ensure that discriminability is never zero.
