...
//v5debug: print statements to show exactly how the beta value influences decision-making.
functions{
...
model {
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    print("At i:",i, "; beta:",beta,", cue[i]:", cue[i], ", choice_outcomes[i,]:",choice_outcomes[i,], ", exp_val[cue[i],]:", exp_val[cue[i],], 
      ", their product:", to_vector(exp_val[cue[i],]) * beta,", response[i]:", response[i], ".");
    for(j in 1:NUM_CHOICES){
