//rl_discrim_single_exp_joint_random_control
//modified from discrim v10a, which was a mixture model, but functionally removes the discrimination componentry, AND any learning, and
//is thus a model of random learning.
//should compare to a simpler model where code is actually stripped down to be random.
//and a slightly more complex model with learning but no discrimination, but derived from the discriminatoin model.

//v11: Mixture model in v10a with separate learning rates in v9.
//v10a: fix bug in v10.
//v10: v5, but with a mixture model.
//
//rl_single_exp_joint
...
//vxa of each of the below: change to examine the RPE bsed on the actual chosen option rather than always the second option.
...
transformed data{
  ...
  int beta=0;//artificially defined.
...
model {
  ...
  int recent_cue_id[n_back];// [n_back];
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    if(trial_n_back==0){
      ...
      for (mi in 1:trial_n_back){
        recent_cue_id[mi] = cue[i-trial_n_back+mi-2];
    ...
    if(trial_n_back==0){
        ...
        target += categorical_logit_lpmf(response[i] | to_vector(ev_over_discrim) * beta);
    }else{
      //the scaled match score is essentially the mixture ratio.
      for (j in 1:NUM_CHOICES){
      ...
      #could treat ev_over_discrim * beta as our lambdas; then...
      for (k in 1:trial_n_back){
        #response[i] ~ categorical_logit( to_vector(ev_over_discrim) * beta);
        lps[k] = log(scaled_match_score[k]) + categorical_logit_lpmf(response[i] | to_vector(exp_val[recent_cue_id[k],]) * beta);
      }
      //print(lps);
      target += log_sum_exp(lps);
    }
    
    //it's gonna be impossible not to just take the average for these expected values, because
    //this part is 
...
generated quantities{
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    trial_n_back=0;//max(min(n_back,i-2),0);
    ...
    if(trial_n_back==0){
        ev_over_discrim = exp_val[cue[i],];
        log_lik += categorical_logit_lpmf(response[i] | to_vector(ev_over_discrim) * beta);
    }else{
      for (j in 1:NUM_CHOICES){
        //ev_over_discrim[j] = sum(exp_val[1:trial_n_back,j].*scaled_match_score[1:trial_n_back]);
        ev_over_discrim[j] = sum(exp_val[recent_cue_id[1:trial_n_back],j].*scaled_match_score[1:trial_n_back]);
