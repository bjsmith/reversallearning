...
model {
  ...
  subj_sigma[PARID_alpha] ~ normal(1,5); 
  //these have lower prior SDs because our priors for them originally, from Palmeri et al., were lower.
  subj_sigma[PARID_lba_k] ~ normal(1,3); 
  subj_sigma[PARID_lba_tau] ~ normal(1,2);
  ...
  for (s in 1:NUM_SUBJECTS){
    ...
    run_sigma[s,PARID_alpha] ~ normal(1,4); 
    run_sigma[s,PARID_lba_k] ~ normal(1,2); 
    run_sigma[s,PARID_lba_tau] ~ normal(1,2);
  }
