//rl_single_exp_parallel
//this does NOT use the lba model. For comparison purposes it is included anyway, but it's unused.
//this estmiates neural and behavioral features, but behavioral parameter is excluded entirely from the DeltaThetas
...
//v4: I don't think the alpha and beta priors changed much at allbut it has come to my attention that beta might be misconfigured?
functions{
...
transformed data{
  ...
  //int TD_ev=THETA_ev;
  int TD_accumbens_lh = DELTA_accumbens_lh;
  int TD_accumbens_rh = DELTA_accumbens_rh;
  int TD_ofc_lh = DELTA_ofc_lh;
  int TD_ofc_rh = DELTA_ofc_rh;
  int TD_N=TD_ofc_rh;
...
model {
  ...
  alpha_pr ~ normal(0,1.8138);//a uniform prior in the logit
  ...
  beta_pr ~ normal(0,6);
    //we were previously using an inverse logit multiplied by a constant. that seemed crazy.
    //I want to keep this as something with a conjugate distribution..
  // k_pr ~ normal(log(.5),5); 
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    //the EXPECTED value for this choice, which *should* always be positive(?) but will differ in degree of how positive you'd expect.
