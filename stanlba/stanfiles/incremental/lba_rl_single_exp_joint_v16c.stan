...
//v16 attempts an entirely new estimator for the joint probability matrix. It's a really naive and uninformative sample for each individual cell of the covariance matrix unrelated to each other.
functions{
...
parameters {
  ...
  //cov_matrix[TD_N] Sigma_pr;
  //matrix[TD_N,TD_N] Sigma_est;
  cholesky_factor_corr[TD_N] Lcorr;  
  vector<lower=0>[TD_N] sigma; 
  // ////////////\end{joint model machinery}
}
...
transformed parameters {
  ...
  // ////////////\begin{joint model machinery}
  //matrix[TD_N, TD_N] Sigma;
  //cov_matrix[TD_N] Sigma;
  matrix[TD_N, TD_N] L_Sigma = diag_pre_multiply(L_sigma, Lcorr);
  //matrix[TD_N, TD_N] Sigma = L_Sigma * L_Sigma';
  //matrix[TD_N, TD_N] Sigma = matrix()
  //print(Sigma_pr);
...
model {
  ...
  vector[TD_N] td_sd;
  // for (i in 1:TD_N){
  //   Sigma_pr[i,] ~ normal(0.0,3.0);
  // }
  // for (i in 1:TD_N){
  //   for (j in 1:TD_N){
  //     if(i!=j){
  //       Sigma_est[i,j] ~ uniform(-1,1);
  //       // Sigma_est[j,i] ~ uniform(-1,1);
  //     // }else{
  //     //   Sigma_est[i,j] ~ uniform(.99,1.01);
  //     }
  //   }
  // }
  ...
  //td_var ~ multi_normal(zeros,Sigma);#multi_normal_cholesky(zeros,L_Sigma);
...
generated quantities{
  matrix[TD_N, TD_N] Sigma;
  matrix[TD_N, TD_N] Omega;
  matrix[TD_N, TD_N] Omega= multiply_lower_tri_self_transpose(Lcorr);
  matrix[TD_N, TD_N] Sigma= quad_form_diag(Omega,L_sigma);
