...
//v8b: build on v8; includes a posterior estimate.
functions{
...
model {
  ...
  //print("mean(response_time):",min(response_time));
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    //response_time[i] = mean(response_time) + 1/(1+e^(Act_a[response[i]]-theta));
    ...
    //"theta_param:", Act_a[response[i]] -log(1/(response_time[i]-min(response_time))-1));
    
    //use the baseline response time that I used as a cutoff, 0.11
    theta~normal(Act_a[response[i]] -log(1/(response_time[i]-0.11)-1),2);
    //or we solve for Act_a and make theta the parameter.
...
generated quantities{
  ...
  real log_lik_rl; //to calculate a proper log probability
  real log_lik_joint;
  real log_lik_opal;
  real log_lik;
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    //   sum(e^(Act_a)));
      //we no longer have beta because we have separate beta_G and beta_N
     log_lik_rl = log_lik_rl + categorical_logit_lpmf(response[i] | Act_a);
    ...
    //use the baseline response time that I used as a cutoff, 0.11
    log_lik_opal = log_lik_opal + normal_lpdf(theta | Act_a[response[i]] -log(1/(response_time[i]-0.11)-1),2);
    //or we solve for Act_a and make theta the parameter.
  ...
  //predict the variance from the remaining information.
  log_lik_joint = log_lik_joint + lkj_corr_cholesky_lpdf(L_Omega | 1);
  //print(L_Omega)
  log_lik_joint = log_lik_joint + cauchy_lpdf(L_sigma | 0,1); //these yield standard deviations of each individual value.
  log_lik_joint = log_lik_joint + multi_normal_cholesky_lpdf(td_var | zeros,L_Sigma);
  ////////////\end{joint model machinery}
  log_lik=log_lik_rl + log_lik_opal + log_lik_joint;
}
