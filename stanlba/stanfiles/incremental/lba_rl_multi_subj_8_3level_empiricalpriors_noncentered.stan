// this version:
  // - fixes an lba problem where very low priors are treated as infinite;
// - no longer prints choice outcomes.
functions{
  ...
  real lba_log(real response_time, int response, real k, real A, vector v, real s, real tau,int subj,int run, int trial){
    ...
    // for (i in 1:rows(RT)){	
      ...
      if(t > 0){			
        ...
        if(prob < 1e-10){
          //print("lba evaluated as a exceedingly unlikely for subject ", subj, ", run ", run, ", trial ", trial)  
          prob = 1e-10;
  ...
  model {
    ...
    subj_mu[PARID_alpha] ~ normal(priors_alpha,priors_alpha_spread);//-3;//~ normal(3,0.01);
  ...
  // print("subj_mu: ",subj_mu,"; priors: ",priors_alpha,",",priors_lba_k,",",priors_lba_tau,"; priors: ",priors_alpha_spread,",",priors_lba_k_spread,",",priors_lba_tau_spread)
  // print("subj_sigma: ",subj_sigma,"; priors: ",priors_alpha_sd_gamma,",",priors_lba_k_sd_gamma,",",priors_lba_tau_sd_gamma)
  // print("alpha mean",mean(alpha))
  // print("k mean ",mean(k))
  // print("tau mean ",mean(tau))
  ...
  for (t in 1:NUM_TRIALS){//loop through timesteps.
    for(j in 1:NUM_CHOICES){
      ...
      response_time[t] ~ lba(response[t],k[trial_runid[t]],A,v,lba_sd,tau[trial_runid[t]],run_subjid[trial_runid[t]],trial_runid[t],t);
  }
}
