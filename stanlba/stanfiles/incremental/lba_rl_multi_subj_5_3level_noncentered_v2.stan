...
transformed parameters {
  ...
  // for (r in 1:NUM_RUNS){
  //   alpha_pr[r] = run_mu[run_subjid[r],PARID_alpha] + run_sigma[run_subjid[r],PARID_alpha] * alpha_pr_var[r];
  //   k_pr[r] = run_mu[run_subjid[r],PARID_k] + run_sigma[run_subjid[r],PARID_k] * k_pr_var[r];
  //   tau_pr[r] = run_mu[run_subjid[r],PARID_tau] + run_sigma[run_subjid[r],PARID_tau] * tau_pr_var[r];
  // }
  // 
...
model {
  ...
  subj_mu[PARID_alpha] ~ normal(-1,3);
  ...
  subj_mu[PARID_lba_tau] ~ normal(log(.3*.9),0.5);
  ...
  for (t in 1:NUM_TRIALS){//loop through timesteps.
    print("alpha: ",alpha[trial_runid[t]],"k: ", k[trial_runid[t]],"tau: ",tau[trial_runid[t]])
    for(j in 1:NUM_CHOICES){
