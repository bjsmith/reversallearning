//v3a: same as v3 but with log_lik generated quantity.
//v3: reverts to v1, then constrains alpha to be a *single value* that applies to learning both the cue identities and the values of the cues.
...
transformed data{
  matrix[2,2] alpha_Sigma_prior=[[2,1],[1,2]];
  real<lower=0> s = 1;
  int n_back=3;
  //from stan manual:
...
transformed parameters {
  ...
  alpha_pr = alpha_mv_pr[1];
  alpha_discrim_pr = alpha_mv_pr[2];
...
model {
  ...
  alpha_mv_pr ~ multi_normal([0,0],alpha_Sigma_prior);//weak prior, agnostic about learning rate, but still biased to low learning rates.
  ...
  for (i in 1:LENGTH){//loop through timesteps.
    ...
    if(trial_n_back>0){
      ...
      //match_score[1:trial_n_back]=inv_logit((cue_match[1:trial_n_back]*2-1).*recent_cue_discriminability[1:trial_n_back]);
      //alternative formula cooked up in Excel.
      match_score[1:trial_n_back]=inv_logit((cue_match[1:trial_n_back]*1.5-0.5).*recent_cue_discriminability[1:trial_n_back]*0.4);
      for (j in 1:trial_n_back){
        match_score_sq[j]=match_score[j]^2;
      }
      scaled_match_score[1:trial_n_back] = match_score[1:trial_n_back]/sum(match_score[1:trial_n_back]);
    }
    // print("recent_cue_familiarity[1:trial_n_back]:", recent_cue_familiarity[1:trial_n_back]);
    // print("recent_cue_discriminability[1:trial_n_back]:", recent_cue_discriminability[1:trial_n_back]);
    // print("cue_match[1:trial_n_back]:                  ",cue_match[1:trial_n_back]);
    // print("match_score[1:trial_n_back]:                 ",match_score[1:trial_n_back]);
    // print("alpha_discrim:                 ",alpha_discrim);
    // print("scaled_match_score:                      ",scaled_match_score);
    // 
