    ...
    if(check.pars(x=x,use.data=use.data)){
      ...
      for (s in 1:length(use.data)){#s<-1#first subject
        ...
        for(tr in 1:nt){#tr=1;tr=tr+1
          ...
          #tstart<-Sys.time()
          library(rbenchmark)
          benchmark(wald.pdf.c(t[idx1],alpha[1],v[idx1,1],theta[1])*(1-wald.cdf.c(t[idx1],alpha[2],v[idx1,2],theta[2])),
                    wald.pdf.raw(t[idx1],alpha[1],v[idx1,1],theta[1])*(1-wald.cdf.raw(t[idx1],alpha[2],v[idx1,2],theta[2])),replications = 100000)
          #x_w=t[idx]-theta
