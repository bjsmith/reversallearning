...
for(j in 1:S){
  for (r in 1:s_runs.N[j]){#j=1;r=1
    ...
    while(temp.weight==-Inf){
      ...
      new.x_s["mash"]<-rbinom(1,1,prior.l2[["mash"]][["mu"]]) #going to have to use the level 2 parameter because there's no distributional info inherent in l1.
      if(!warned.dist1){
    ...
    #init.pars[j,r,]=(1-a)*new.x_s+(a)*optim(new.x_s,function(x_s,data)-log.dens.like(x_s,data),data=data[[j]]$runs[[r]])$par
  ...
  for(j in 1:S){#j<-1
    for (r in 1:s_runs.N[j]){#r<-1
      ...
      while(weight[i,j,r]==-Inf){
        ...
        theta[i,param.l1.ids$mash,j,r]= i<=round(prior.l2$mash$p*n.chains)
          #we want to ensure the chains are 'salted' with this variable because it's binomial.
          #so this ensures that a few chains start go each way.
          #this will enable DE-MCMC to operate properly.
    ...
    for(k in (param.l2.distributions.N+1):(2*param.l2.distributions.N-1)) {
