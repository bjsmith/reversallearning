...
log.dens.prior.l2=function(x_s,        #  actually these are phi, just a lower level phi
  ...
  dens=sum(dnorm(x_s[param.l2.ids[["alpha_s_mu"]]],use.phi[param.l3.ids[["alpha_s_mu_g_mu"]]],use.phi[param.l3.ids[["alpha_s_mu_g_sigma"]]],log=TRUE)) + 
    ...
    sum(2*dcauchy(x_s[param.l2.ids[["alpha_s_sigma"]]],0,prior.l2$alpha$sigma_gamma,log=TRUE)) + 
    sum(2*dcauchy(x_s[param.l2.ids[["thresh_s_sigma"]]],0,prior.l2$thresh$sigma_gamma,log=TRUE)) +
    sum(2*dcauchy(x_s[param.l2.ids[["tau_s_sigma"]]],0,prior.l2$tau$sigma_gamma,log=TRUE))
    #up until version j, the prior for tau_s_sigma was inappropriately set to thresh_sigma
    #however this won't affect anything because both tau and thresh were set to 10, as weakly informative priors.
...
log.dens.like=function(x_s,use.data_s,method="full"){
  ...
  if(check.pars(x=x,use.data_s=use.data_s)){
    ...
    dens.v<-log(get.dens.2choice(t=use.data_s$rt[use.data_s$choice!=0],
