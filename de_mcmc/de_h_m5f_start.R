...
for(j in 1:S){
  for (r in 1:s_runs.N[j]){#j=1;r=1
    ...
    new.x_s=x_s
    a<- -1 #gotta try 100 iterations of random generation before we try mixing in an optimized value.
    while(temp.weight==-Inf){
      new.x_s["alpha"]<-rnorm(1,-3,2)#we start alpha low, as a conservative prior.
      new.x_s["thresh"]<-rnorm(1,x_s["thresh"],abs(x_s["thresh"]))
      new.x_s["tau"]<-rnorm(1,x_s["tau"],abs(x_s["tau"]))
