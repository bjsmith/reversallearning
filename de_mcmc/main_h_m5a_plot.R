...
plot.weights=FALSE
plot.priors=FALSE
load("/expdata/bensmith/joint-modeling/data/msm/reversallearning/de_mcmc/output_h_m5d20180519T202105.RData")
...
for(q in 1:n.chains){#q<-1
  if(plot.lower==TRUE){
    for(s in 1:S){
      ...
      if(run.ts=="20180510T132805"){
        temp=t(as.matrix(read.table(paste("chain",q,"_sub",s,"_run",1,".txt",sep=""),header=F)))
        #theta[q,,s,,]=aperm(array(temp,c(R_max,tnmc,param.N)),perm = c(3,1,2))
        theta[q,,s,,]=array(temp,c(R_max,tnmc,param.N))#no need to aperm; this happens to be in the right order!
      }else {
        stop("not implemented yet. take a look!")
  ...
  if(plot.phi.s==TRUE){temp=t(as.matrix(read.table(paste("chain",q,"_hyper_phi_s.txt",sep=""),header=F)));phi.s[q,,]=temp[,-1]}
  #let's get it straight how to interpret this. dim1 represents params*groups.
  
  if(plot.phi.g==TRUE){temp=t(as.matrix(read.table(paste("chain",q,"_hyper_phi_g.txt",sep=""),header=F)));phi.g[q,,]=temp[,-1]}
...
for (phi.x in phi_vals){
  ...
  for(k in rep((0:(dim(phi.x)[2]/6-1))*6,each=3)+rep(1:3,times=dim(phi.x)[2]/6)){#loop through each of the params, and at each iteration, do both the central dtendency and deviancy param.
    ...
    for (hyper_type in c(0,3)){
      ...
      k_t<-k+hyper_type
      matplot(t(phi.x[,k_t,start:tnmc]),type="l",lty=1,main="",ylab=dimnames(phi.x)[[2]][k_t])
      hist(phi.x[,k_t,start:tnmc],prob=T,breaks=breaks,main="",xlab=dimnames(phi.x)[[2]][k_t])
    }
    if(plot.priors==TRUE){
      lines(xs,ys,lty=2)
      count=count+1
  ...
  if(is.prime(t_s)){
    ...
    for (r in 1:s_runs.N[t_s]){
      ...
      for(k in 1:(dim(theta)[2])){#parameters
        ...
        matplot(t(theta[,k,t_s,r,start:tnmc]),type="l",lty=1, main=theta_val, ylab=dimnames(theta)[[2]][k])
        hist(theta[,k,t_s,r,start:tnmc],prob=T,breaks=breaks, main=theta_val, xlab=dimnames(theta)[[2]][k])
        if(plot.priors==TRUE){
          lines(xs,ys,lty=2)
          count=count+1
        }
