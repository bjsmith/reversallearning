...
param.key<-rbind(data.frame("level"=3,"varname"=paste0(rep(dimnames(phi_g)[[2]],times=length(dimnames(phi_g)[[3]])),"_",rep(dimnames(phi_g)[[3]],each=length(dimnames(phi_g)[[2]])))),
                 ...
                 data.frame("level"=1,"varname"=paste0(rep(param.l1.names,times=R_max),"_r",rep(1:R_max,each=length(param.l1.names)))))
...
for(i in 2:nmc){#loop through iterations#i<-2
  ...
  #We are struggling HERE, in calculating weights/thetas at the run level
  temp=#sfLapply(
    lapply( 
    1:n.grid,
  ...
  last_time<-Sys.time()
  if(last_time-20>last_time_print_progress){
    print(t(as.data.frame(diagnostic_record)))
    #print(diagnostic_record)
