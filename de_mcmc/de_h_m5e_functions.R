...
log.dens.prior=function(x_s,
  ...
  dens.prior.code<-function(){
    ...
    sum(dbinom(x_s[["mash"]],1,use.phi[param.l2.ids[["mash_s_p"]]],log=TRUE))
  }
...
log.dens.prior.l2=function(x_s,        #  actually these are phi, just a lower level phi
  ...
  dens=sum(dnorm(x_s[param.l2.ids[["alpha_s_mu"]]],use.phi[param.l3.ids[["alpha_s_mu_g_mu"]]],use.phi[param.l3.ids[["alpha_s_mu_g_sigma"]]],log=TRUE)) + 
    ...
    sum(2*dcauchy(x_s[param.l2.ids[["alpha_s_sigma"]]],0,5,log=TRUE)) + 
    sum(2*dcauchy(x_s[param.l2.ids[["thresh_s_sigma"]]],0,5,log=TRUE)) +
    sum(2*dcauchy(x_s[param.l2.ids[["tau_s_sigma"]]],0,5,log=TRUE))
...
log.dens.like=function(x_s,use.data_s,method="full"){
  ...
  if(check.pars(x=x,use.data_s=use.data_s)){
    ...
    #probability that the subject is actually just random button-mashing,
    #with a simplifying assumption that we know the probability with which they change their mashing frequency
    if(x_s[param.l1.ids$mash]==1){
      choice_series<-use.data_s$choice[use.data_s$choice!=0]
      changeDetect<-function(vec){vec[2:length(vec)]!=vec[1:(length(vec)-1)]}
      p_switch=sum(changeDetect(use.data_s$choice))/(length(use.data_s$choice)-1)
      changes<-changeDetect(choice_series)
      out=log(((p_switch)^sum(changes))*((1-p_switch)^sum(!changes)))
    }else{
...
log.dens.like.l2=function(x_s,use.data,method="full"){
  #if use.data contains four indices, knock it down to 3
