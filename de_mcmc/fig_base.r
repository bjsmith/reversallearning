...
if(plot.sigma==TRUE){
  par(mfrow=c(2,2),ask=T)
  for(k in 1:n.Sigma1){
    matplot(t(Sigma1[,k,start:tnmc]),type="l",lty=1,main=paste("Sigma1",k))
    hist(Sigma1[,k,start:tnmc],prob=T,breaks=breaks)
  }
  for(k in 1:n.Sigma2){
    matplot(t(Sigma2[,k,start:tnmc]),type="l",lty=1,main=paste("Sigma2",k))
    hist(Sigma2[,k,start:tnmc],prob=T,breaks=breaks)
  }
}
...
if(plot.mu==TRUE){
  par(mfrow=c(2,2),ask=T)
  for(k in 1:n.mu1){
    matplot(t(mu1[,k,start:tnmc]),type="l",lty=1,main=paste("mu1",k))
    hist(mu1[,k,start:tnmc],prob=T,breaks=breaks)
  }
  for(k in 1:n.mu2){
    matplot(t(mu2[,k,start:tnmc]),type="l",lty=1,main=paste("mu2",k))
    hist(mu2[,k,start:tnmc],prob=T,breaks=breaks)
  }
}
...
if(plot.rho==TRUE){
  ...
  shift=n.mu1
  for(k in 2:shift){
    temp=Sigma1[,k,start:tnmc]/(sqrt(Sigma1[,shift*(k-1)+k,start:tnmc])*sqrt(Sigma1[,1,start:tnmc]))
    matplot(t(temp),type="l",lty=1,main=paste("Sigma1",k))
    hist(temp,prob=T,breaks=breaks)
  
    temp=Sigma2[,k,start:tnmc]/(sqrt(Sigma2[,shift*(k-1)+k,start:tnmc])*sqrt(Sigma2[,1,start:tnmc]))
