...
#set settings.
models_to_run<-c("double_update_nov_rev2-a-a",#"double_update_nov_rev2-d",#"double_update_nov_rev2-c",
                 "double_update_notrialpost")
estimation_methods<-c(as.character(ESTIMATION_METHOD.MCMC))#rev(ESTIMATION_METHODS)
...
times_to_run<-1
#run.
summaryfilepath<-paste0(localsettings$data.dir,"du_model_revised_4runs_20171206_2aa_1st_iter.RData")
          ...
          for (rp in rp_list){
            for(runs in runlist){
              ...
              fitpackage<-lookupOrRunFit(
                ...
                fastDebug=FALSE,
                fileSuffix=paste0("rev2aa_20171206_test",as.character(t)),
                estimation_method = em,
                bseed=t+202834755,#set.seed(as.numeric(Sys.time())); sample.int(.Machine$integer.max-1000, 1)
