...
estimation_methods<-c(as.character(ESTIMATION_METHOD.DEMCMC))
        ...
        for (g in subject_groups){
          ...
          for (rp in rp_list){
            for(runs in runlist){
              ...
              if(em==as.character(ESTIMATION_METHOD.DEMCMC)){
                print("code to summarize DEMCMC analysis goes here.")
                model.summaries[[first_empty_list_pos]]<-list("note"="Code to summarize DEMCMC analysis hasn't been written yet!",
